...
class BST:
    ...
    def heading_root_and_null_make(self):# By default window presentation
        ...
        make_root = Label(self.make_canvas,text="Root",font=("Arial",17,"bold"),fg="#00FF00",bg="chocolate")
    ...
    def make_instructional_buttons(self):# For make Instructional buttons
        #Insert button
        self.insert_node = Button(self.window,text="Insert",font=("Arial",20,"bold"),bg="black",fg="red",relief=RAISED,bd=8,command=lambda: self.take_input_set_up(1),state=NORMAL)
        ...
        self.pre_order_traversal  = Button(make_label_frame_for_traversing,text="Pre-order",font=("Arial",10,"bold"),bg="black",fg="red",relief=RAISED,bd=8,command=lambda: self.traversing_decision_making(1),state=NORMAL)
        self.pre_order_traversal.pack(padx=10,pady=5)

        self.in_order_traversal = Button(make_label_frame_for_traversing, text="In-order", font=("Arial", 10, "bold"),bg="black", fg="red", relief=RAISED, bd=8, command=lambda: self.traversing_decision_making(2), state=NORMAL)
        self.in_order_traversal.pack(padx=10, pady=5)

        self.post_order_traversal = Button(make_label_frame_for_traversing, text="Post-order", font=("Arial", 10, "bold"), bg="black", fg="red", relief=RAISED, bd=8, command=lambda: self.traversing_decision_making(3), state=NORMAL)
        self.post_order_traversal.pack(padx=10, pady=5)

        self.level_order_traversal = Button(make_label_frame_for_traversing, text="Level-order Display", font=("Arial", 10, "bold"), bg="black", fg="red", relief=RAISED, bd=8, command=lambda: self.traversing_decision_making(4), state=NORMAL)
        ...
        self.level_order_display = Button(make_label_frame_for_traversing, text="Level-order Concept",
                                            font=("Arial", 10, "bold"), bg="black", fg="red", relief=RAISED, bd=8,
                                            command=self.level_order_concept_by_queue, state=NORMAL)
        self.level_order_display.pack(padx=3, pady=5)
        # Delete Button
        self.make_delete = Button(self.window, text="Delete", font=("Arial", 16, "bold"), bg="black", fg="red", relief=RAISED, bd=8, command=lambda: self.take_input_set_up(2), state=NORMAL)
    ...
    def filtration_of_input_value(self, e, indicator): # Filtration with button deactivation and activation
        ...
        try:
            ...
            messagebox.showerror("Value input error","Input value must be integer")
    ...
    def make_default_node_with_set_position(self):# For node make and set at right position in canvas
        try:
            ...
            self.make_node = self.make_canvas.create_oval(505, 85+50, 555, 135+50, width=2, fill="black", outline="blue")
        ...
        if self.notation_index == 0:
           ...
           for take in self.node_number_value_store:
               if int((self.notation_index-1)/2) == take[3]:# Parent node notation index take searching in actual list
                     ...
                     self.take_arrow = self.make_canvas.create_line(arrow_coord, width=3, fill="red")
                     break
    ...
    def pre_order_traversing(self,control_node):
        if control_node is None:
            ...
            self.make_canvas.itemconfig(control_node[0],fill="green",outline="black")
            self.display_box_counter += 1
            self.value_show[self.display_box_counter].config(text=control_node[2])
            control_node[1].config(bg="green",fg="yellow")
            self.window.update()
    ...
    def left_none_right_exist(self,node):
        ...
        if left_side_existence_checking_of_one_side_down_label_node == 1:
            ...
            if temp1:
                ...
                make_circle = self.make_canvas.create_oval(node_coord, width=3, fill="black", outline="blue")

                arrow_coord = self.make_left_arrow_for_node_left_manually(node_coord, p_node_coord)

                take_arrow = self.make_canvas.create_line(arrow_coord, width=3, fill="red")

                label_make = Label(self.window, text=take_temp1_val, fg="red", bg="black",
                                   font=("Arial", 12, "bold"))
    ...
    def right_none_left_exist(self, node):
        ...
        if  right_side_existence_checking_of_one_side_down_label_node == 1:
            ...
            if temp1:
                ...
                label_make = Label(self.window, text=take_temp1_val, fg="red", bg="black", font=("Arial", 12, "bold"))
                label_make.place(x=node_coord[0] + 14, y=node_coord[1] + 14)
    ...
    def color_indicator_to_delete(self,node,temp):# Color indicating to deleted and replaced node
        ...
        self.make_canvas.itemconfig(temp[0], fill="yellow")
        temp[1].config(bg="yellow", fg="green")
        self.make_canvas.itemconfig(node[0], fill="green")
        node[1].config(bg="green", fg="yellow")

        self.status_note.config(text="Green colored node will deleted and replaced by yellow colored node")
        self.status_note.place(x=250, y=130)
        self.window.update()
        time.sleep(5)

        self.make_canvas.itemconfig(temp[0], fill="black", outline="blue")
        temp[1].config(bg="black", fg="red")
        self.make_canvas.itemconfig(node[0], fill="black", outline="blue")
        node[1].config(bg="black", fg="red")
        self.status_note.config(text="Work done")
        self.status_note.place(x=550, y=130)
